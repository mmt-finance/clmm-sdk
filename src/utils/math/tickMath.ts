...
export function convertI32ToSigned(num: number) {
...
export class TickMath {
  ...
  static priceToTickIndexWithTickSpacing(
    price: Decimal,
    decimalsA: number,
    decimalsB: number,
    tickSpacing: number,
  ): number {
    return TickMath.sqrtPriceX64ToTickIndexWithTickSpacing(
      TickMath.priceToSqrtPriceX64(price, decimalsA, decimalsB),
      tickSpacing,
    );
  }
  ...
  static sqrtPriceX64ToTickIndexWithTickSpacing(sqrtPriceX64: BN, tickSpacing: number): number {
    ...
    if (tickLow === tickHigh) {
      return convertSignedToI32(tickLow);
    }
    const derivedTickHighSqrtPriceX64 = TickMath.tickIndexToSqrtPriceX64WithTickSpacing(
      tickHigh,
      tickSpacing,
    );
    if (derivedTickHighSqrtPriceX64.lte(sqrtPriceX64)) {
      return convertSignedToI32(tickHigh);
    }
    return convertSignedToI32(tickLow);
  }
...
export function getTickDataFromUrlData(ticks: any) {
  ...
  for (const tick of ticks) {
    const td: any = {
      ...
      liquidityNet: new BN(
        BigInt.asIntN(128, BigInt(BigInt(tick.liquidityNet.toString()))).toString(),
      ),
